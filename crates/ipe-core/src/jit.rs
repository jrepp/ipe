...
impl JitCode {
    ...
    pub unsafe fn execute(&self, ctx: *const EvaluationContext) -> bool {
        let func: extern "C" fn(*const EvaluationContext) -> u8 = 
...
impl JitCompiler {
    pub fn new() -> Result<Self> {
        ...
        flag_builder.set("opt_level", "speed").map_err(|e| {
            Error::JitError(format!("Failed to set optimization level: {}", e))
        })?;
        
        flag_builder.set("is_pic", "false").map_err(|e| {
            Error::JitError(format!("Failed to disable PIC: {}", e))
        })?;
        ...
        for (idx, instr) in policy.code.iter().enumerate() {
            ...
            match instr {
                ...
                Instruction::Compare { op } => {
                    let b = value_stack.pop().ok_or_else(|| {
                        Error::JitError("Stack underflow in Compare".to_string())
                    })?;
                    let a = value_stack.pop().ok_or_else(|| {
                        Error::JitError("Stack underflow in Compare".to_string())
                    })?;
                    
                    let result = match op {
                        crate::bytecode::CompOp::Eq => {
                            builder.ins().icmp(IntCC::Equal, a, b)
                        }
                        crate::bytecode::CompOp::Neq => {
...
mod tests {
    use super::*;
    use crate::bytecode::{Instruction, CompiledPolicy, PolicyHeader, Value, CompOp};
